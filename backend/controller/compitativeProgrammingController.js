...
exports.saveCodeChefUserName = catchAsync(async (req, res, next) => {
  ...
  try {
    const { data } = await axios.get(
      `${process.env.CP_API_BASE_LINK}/codechef/${username}`
    );
    ...
    return next(
      new AppError(
        'Something went wrong with Compitative programming API! Please try again later',
        500
      )
    );
  }
...
exports.getCodeChefData = catchAsync(async (req, res, next) => {
  ...
  const { data } = await axios.get(
    `${process.env.CP_API_BASE_LINK}/codechef/${codechefusername}`
  );
  return res.status(201).json({
    status: 'success',
    data: {
...
exports.saveSPOJUserName = catchAsync(async (req, res, next) => {
  ...
  try {
    const { data } = await axios.get(
      `${process.env.CP_API_BASE_LINK}/spoj/${username}`
    );
    if (data.status === 'Success') {
      const body = {
...
exports.getSpojData = catchAsync(async (req, res, next) => {
  ...
  const { data } = await axios.get(
    `${process.env.CP_API_BASE_LINK}/spoj/${spojUsername}`
  );
  return res.status(201).json({
    status: 'success',
    data: {
...
exports.saveCodeForcesUserName = catchAsync(async (req, res, next) => {
  ...
  try {
    const { data } = await axios.get(
      `${process.env.CP_API_BASE_LINK}/codeforces/${username}`
    );
    if (data.status === 'Success') {
      const body = {
        codeforcesAccount: username,
...
exports.getCodeforcesData = catchAsync(async (req, res, next) => {
  ...
  const { data } = await axios.get(
    `${process.env.CP_API_BASE_LINK}/codeforces/${codeforces}`
  );
  if (data.status === 'Failed') {
    return next(
      new AppError("You don't have valid Codeforces username Please update ")
    );
  }
  return res.status(201).json({
    status: 'success',
